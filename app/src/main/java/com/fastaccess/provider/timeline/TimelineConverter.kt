        ...
        return Observable.fromIterable(jsonObjects)
            .map { jsonObject ->
                ...
                if (!InputHelper.isEmpty(event)) {
                    ...
                    when (type) {
                        IssueEventType.COMMENTED -> timeline.comment = getComment(jsonObject, gson)
                        else -> timeline.genericEvent = getGenericEvent(jsonObject, gson)
                    }
        ...
        jsonObjects.onEach { jsonObject ->
            ...
            if (!InputHelper.isEmpty(event)) {
                ...
                when (type) {
                    ...
                    IssueEventType.COMMIT_COMMENTED -> {
                    ...
                    IssueEventType.REVIEWED, IssueEventType.CHANGES_REQUESTED -> {
                        ...
                        if (review != null) {
                            ...
                            comments?.items
                                ?.filter { it.pullRequestReviewId == review.id }
                                ?.onEach {
                            ...
                            comments?.items
                                ...
                                ?.onEach {
                                    ...
                                    reviewsList.onEach { reviews ->
                                        if (it.path == reviews.groupedReviewModel?.path &&
